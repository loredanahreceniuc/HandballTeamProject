...
namespace exp.NET6.Infrastructure.Repositories.TeamCategories
{
    ...
    {
        public TeamCategoryRepository(FlowerPowerDbContext context): base(context) { }

        public async Task<TeamCategory> DeleteVirtual(int id)
        {
            try
            {
                var entity = await context.Set<TeamCategory>().FindAsync(id);
                if (entity == null)
