...
namespace exp.NET6.Infrastructure.Repositories.BlogCategories
{
    ...
    {
        public BlogCategoryRepository(FlowerPowerDbContext context) : base(context) { }

        public async Task<BlogCategory> DeleteVirtual(int id)
        {
            try
            {
                var entity = await context.Set<BlogCategory>().FindAsync(id);
                if (entity == null)
